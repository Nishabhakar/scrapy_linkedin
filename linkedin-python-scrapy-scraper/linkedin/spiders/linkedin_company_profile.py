...
class LinkedCompanySpider(scrapy.Spider):
    name = "linkedin_company_profile"
    api_url = 'https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords=python&location=United%2BStates&geoId=103644278&trk=public_jobs_jobs-search-bar_search-submit&start=25'

    def start_requests(self):
        yield scrapy.Request(url=self.api_url, callback=self.parse_search_results)

    def parse_search_results(self, response):
        company_urls = response.css('.search-results__list .result-card__full-card-link::attr(href)').getall()
